...
class RMProFormaModel:
    ...
    def _initialize_yield_curves(self):
        ...
        for year in range(5):
            ...
            year_rates = [r + self.inputs['goingOnYields'][year]['loans'] for r in base_rates]
    ...
    def _generate_monthly_schedule(self):
        ...
        # Adjust the index to month-end dates
    ...
    def _generate_annual_summary(self, monthly_schedule):
        ...
        logger.debug("Starting _generate_annual_summary")
        logger.debug(f"Input monthly_schedule: {monthly_schedule}")

        # Define income statement and balance sheet items
        income_statement_items = [
            'interest_income_loans', 'interest_income_lines', 'interest_expense', 
            'non_interest_income', 'non_interest_expense', 'provision_expense', 
        ...
        for column in monthly_schedule.columns:
            logger.debug(f"Column {column} sum: {monthly_schedule[column].sum()}")
            logger.debug(f"Column {column} non-zero count: {(monthly_schedule[column] != 0).sum()}")

        # Aggregate income statement items by summing them annually
        annual_summary[income_statement_items] = monthly_schedule[income_statement_items].resample('A').sum()
        ...
        denominator = (
            ...
            annual_summary['unused_commitment_fees'] - 
