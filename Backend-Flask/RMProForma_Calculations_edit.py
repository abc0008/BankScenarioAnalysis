...
class RMProFormaModel:
    ...
    def validate_inputs(self):
        ...
        required_fields = ['rmHireDate', 'firstProductionDate', 'annualProduction', 'loanVsLinePercentage', 
                           ...
                           'averageLifeLines', 'prepayPercentageOfBalance']
    ...
    def _generate_monthly_schedule(self):
        ...
        df = pd.DataFrame(index=self.dates, columns=[
            ...
            'salary_expense', 'benefits_expense', 'discretionary_expense',
    ...
    def _calculate_efficiency_ratio(self, row):
        """Calculate efficiency ratio as non-interest expense divided by total revenue."""
        total_revenue = row['interest_income_loans'] + row['interest_income_lines'] + row['non_interest_income']
        if total_revenue == 0:
            return float('inf')
        return (row['non_interest_expense'] / total_revenue) * 100
    ...
    def _generate_annual_summary(self, monthly_schedule):
        ...
        annual_summary = monthly_schedule.resample('YE').agg({
            'loan_balance': 'last',
            'line_balance': 'last',
            'deposit_balance': 'last',
            'interest_income_loans': 'sum',
            'interest_income_lines': 'sum',
            ...
            'total_assets': 'last',
            'total_liabilities': 'last',
            'total_equity': 'last',
            'salary_expense': 'sum',
            'benefits_expense': 'sum',
            'discretionary_expense': 'sum',
            'origination_fees': 'sum',
            'unused_commitment_fees': 'sum'
        })
    
        annual_summary['return_on_assets'] = annual_summary['net_income'] / annual_summary['total_assets']
        annual_summary['return_on_equity'] = annual_summary['net_income'] / annual_summary['total_equity']
        annual_summary['total_interest_income'] = annual_summary['interest_income_loans'] + annual_summary['interest_income_lines']
        ...
        return annual_summary.reset_index()
    ...
    def calculate_pro_forma(self):
        ...
        try:
            ...
            return {
                'monthlySchedule': monthly_schedule.reset_index().to_dict(orient='records'),
                'annualSummary': annual_summary.to_dict(orient='records'),
                'totalMetrics': total_metrics,
                'cumulativePayback': cumulative_payback,
            }
